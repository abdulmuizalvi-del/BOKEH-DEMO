...
const TIMELINE_EVENTS = [
  { id: 'prep', time: '09:00 AM', title: 'Preparation', location: 'Bridal Suite — King Edward Hotel', description: 'Getting ready shots, bridal party, details', photoIndices: [0, 4, 8], color: 'fuchsia' },
  { id: 'ceremony', time: '12:30 PM', title: 'Ceremony', location: "St. Michael's Cathedral, Toronto", description: 'Vows, rings, first kiss, family portraits', photoIndices: [1, 2, 5], color: 'violet' },
  { id: 'reception', time: '06:00 PM', title: 'Reception', location: 'Arcadian Court, Bay Street', description: 'Cocktail hour, first dance, toasts, party', photoIndices: [3, 6, 7], color: 'teal' },
]
...
export default function ClientProject() {
  ...
  return (
    <>
      <button onClick={() => router.push('/discover')} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-white transition-colors mb-6 group">
      ...
      <div className="mb-8">
        ...
        <div className="inline-flex items-center gap-1 p-1.5 bg-white/5 border border-white/10 rounded-2xl">
          {TABS.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`relative px-5 py-2 text-sm font-semibold rounded-xl transition-all duration-200 ${activeTab === tab ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'text-white/50 hover:text-white/80'}`}>
              {tab}
            </button>
          ))}
        </div>
              ...
              <div className="relative p-6 flex flex-col md:flex-row md:items-center gap-6">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-4 mt-2 text-sm text-muted-foreground">
                    <span className="flex items-center gap-1.5"><Camera className="w-3.5 h-3.5 text-fuchsia-400" />{MOCK_PROJECT.photographer}</span>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  ...
                  <div key={label} className={`flex items-center gap-4 p-4 rounded-xl border ${color === 'green' ? 'border-green-500/20 bg-green-500/5' : color === 'teal' ? 'border-teal-500/20 bg-teal-500/5' : 'border-fuchsia-500/20 bg-fuchsia-500/5'}`}>
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${color === 'green' ? 'bg-green-500/15' : color === 'teal' ? 'bg-teal-500/15' : 'bg-fuchsia-500/15'}`}>
                      <Icon className={`w-5 h-5 ${color === 'green' ? 'text-green-400' : color === 'teal' ? 'text-teal-400' : 'text-fuchsia-400'}`} />
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">{label}</p>
                      <p className={`font-bold text-sm ${color === 'green' ? 'text-green-400' : color === 'teal' ? 'text-teal-400' : 'text-fuchsia-400'}`}>{status}</p>
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  {[
                    ...
                    <button key={label} onClick={action} className="w-full flex items-center justify-between p-4 rounded-xl bg-card border border-white/10 hover:border-white/20 hover:bg-white/5 transition-all group">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${accent === 'fuchsia' ? 'bg-fuchsia-500/15' : accent === 'violet' ? 'bg-violet-500/15' : 'bg-teal-500/15'}`}>
                          <Icon className={`w-4 h-4 ${accent === 'fuchsia' ? 'text-fuchsia-400' : accent === 'violet' ? 'text-violet-400' : 'text-teal-400'}`} />
                        </div>
                ...
                <div className="bg-card border border-white/10 rounded-xl overflow-hidden">
                  {[
                    ...
                    <div key={label} className={`flex items-center gap-3 p-4 ${i < arr.length - 1 ? 'border-b border-white/5' : ''}`}>
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${color === 'fuchsia' ? 'bg-fuchsia-500/15' : color === 'green' ? 'bg-green-500/15' : color === 'teal' ? 'bg-teal-500/15' : 'bg-violet-500/15'}`}>
                        <Icon className={`w-3.5 h-3.5 ${color === 'fuchsia' ? 'text-fuchsia-400' : color === 'green' ? 'text-green-400' : color === 'teal' ? 'text-teal-400' : 'text-violet-400'}`} />
                      </div>
          ...
          <motion.div key="galleries" ref={galleryRef} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6">
            ...
            <div className="relative rounded-2xl overflow-hidden group cursor-pointer" style={{ height: '55vh' }}>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 flex items-end justify-between opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button onClick={() => toggleLike(heroIdx)} className={`w-11 h-11 rounded-xl flex items-center justify-center border transition-all ${likedPhotos.has(heroIdx) ? 'bg-rose-500/30 border-rose-500/60 text-rose-400' : 'bg-black/40 backdrop-blur border-white/20 text-white'}`}>
              ...
              {MOCK_PHOTOS.map((url, i) => {
                ...
                return (
                  <motion.div key={i} onClick={() => setHeroIdx(i)}
                    animate={isHighlighted ? { boxShadow: ['0 0 0px rgba(217,70,239,0)', '0 0 25px rgba(217,70,239,0.8)', '0 0 0px rgba(217,70,239,0)'] } : {}}
                    transition={{ duration: 0.8, repeat: isHighlighted ? 3 : 0 }}
                    className={`relative rounded-xl overflow-hidden cursor-pointer transition-all duration-300 aspect-square group ${isHero ? 'ring-2 ring-fuchsia-500 shadow-[0_0_15px_rgba(217,70,239,0.4)]' : isHighlighted ? 'ring-2 ring-fuchsia-400' : 'ring-1 ring-white/10 hover:ring-white/30'}`}
                  >
                    <img src={url} alt={`photo ${i + 1}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    <button onClick={e => { e.stopPropagation(); toggleLike(i) }} className={`absolute top-1.5 right-1.5 w-6 h-6 rounded-full flex items-center justify-center transition-all opacity-0 group-hover:opacity-100 ${likedPhotos.has(i) ? 'bg-rose-500/80 text-white opacity-100' : 'bg-black/50 text-white'}`}>
                      ...
                      <div className={`flex-1 bg-card border rounded-2xl p-5 transition-all duration-300 ${c.border}`}>
                        <div className="flex flex-col md:flex-row md:items-start gap-4">
                          <div className="flex-1">
                            ...
                            <button onClick={() => exploreShots(event.photoIndices)} className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-sm font-semibold transition-all ${c.btn}`}>
                              <ImageIcon className="w-4 h-4" />Explore these shots<ChevronRight className="w-3.5 h-3.5" />
                            </button>
                          </div>
                          <div className="flex gap-2 shrink-0">
                            {event.photoIndices.map((pi, i) => (
                              <div key={i} onClick={() => { setHeroIdx(pi); setActiveTab('My Galleries') }} className={`w-20 h-20 rounded-xl overflow-hidden ring-1 ${c.ring} cursor-pointer hover:opacity-80 transition-opacity`}>
            ...
            <div className="bg-card border border-white/10 rounded-2xl overflow-hidden">
              <div className="flex items-center justify-between p-5 border-b border-white/10">
                <div className="flex items-center gap-3">
                  ...
                  <div><h3 className="font-bold text-white">Wedding Photography Agreement</h3><p className="text-xs text-muted-foreground">Signed • Jan 15, 2027</p></div>
                </div>
                <div className="flex items-center gap-2 px-3 py-1.5 bg-green-500/10 border border-green-500/30 rounded-full">
              ...
              <div className="p-5 space-y-3">
                ...
                {[
                  ...
                  <div key={name} className="flex items-center justify-between p-3 rounded-xl bg-black/30 border border-white/5">
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${signed ? 'bg-green-500/15' : 'bg-amber-500/15'}`}><Pen className={`w-3.5 h-3.5 ${signed ? 'text-green-400' : 'text-amber-400'}`} /></div>
                      <div><p className="text-sm font-semibold text-white">{name}</p><p className="text-xs text-muted-foreground">{role}</p></div>
                    </div>
            ...
            <div className="bg-card border border-white/10 rounded-2xl overflow-hidden">
              <div className="flex items-center justify-between p-5 border-b border-white/10">
                <div className="flex items-center gap-3">
                  ...
                  <div><h3 className="font-bold text-white">Payment Summary</h3><p className="text-xs text-muted-foreground">Wedding Photography Package</p></div>
                </div>
