  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center relative overflow-hidden bg-background">
      ...
      >
        ...
        {/* Role Selector */}
        <div className="flex gap-2 p-1 bg-black/40 rounded-xl border border-white/10 mb-6">
          <button
            type="button"
            onClick={() => setRole('creator')}
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-semibold transition-all ${
              role === 'creator'
                ? 'bg-gradient-primary text-white shadow-lg shadow-fuchsia-500/20'
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="w-full mt-6 py-3 rounded-xl bg-gradient-primary text-white font-semibold flex items-center justify-center gap-2 hover:opacity-90 hover:shadow-[0_0_20px_rgba(217,70,239,0.4)] transition-all"
          >
            Sign In as {role === 'creator' ? 'Creator' : 'Client'}
