...
import {
  Sparkles, Download, Share2, Heart, Plus, Check,
  CheckCircle2, Clock, MapPin, MessageSquare, FileText,
  Camera, Calendar, CreditCard, Eye, ChevronRight,
  Image as ImageIcon, AlertCircle, ExternalLink, Pen
} from "lucide-react";
...
export default function ProjectDetail() {
  ...
  if (!project) return <AppLayout><div className="text-white">Project not found</div></AppLayout>;
    ...
    <AppLayout>
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          <span className="text-xs font-semibold text-fuchsia-400 uppercase tracking-widest">Commission • {project.date}</span>
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="overview" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6">
            ...
            <div className="relative rounded-2xl overflow-hidden border border-white/10 bg-card">
              <div className="absolute inset-0">
                <img src={project.coverUrl} className="w-full h-full object-cover opacity-20" alt="cover" />
                <div className="absolute inset-0 bg-gradient-to-r from-background via-background/90 to-background/60" />
              </div>
              <div className="relative p-6 flex flex-col md:flex-row md:items-center gap-6">
                <img src={project.coverUrl} className="w-24 h-24 rounded-2xl object-cover shrink-0 ring-2 ring-white/10 shadow-xl" alt={project.title} />
                <div className="flex-1">
                  <h2 className="text-2xl font-display font-bold text-white">{project.title}</h2>
                  <div className="flex flex-wrap gap-4 mt-2 text-sm text-muted-foreground">
                    <span className="flex items-center gap-1.5"><Camera className="w-4 h-4 text-fuchsia-400" /> Sarah Chen</span>
                    <span className="flex items-center gap-1.5"><Calendar className="w-4 h-4 text-violet-400" /> {project.date}</span>
                    <span className="flex items-center gap-1.5"><MapPin className="w-4 h-4 text-teal-400" /> Toronto, Canada</span>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <ImageIcon className="w-4 h-4 text-muted-foreground" />
                  <span className="text-white font-bold text-lg">{project.imageCount}</span>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  ...
                  { label: "Gallery", status: project.status === "COMPLETED" ? "Delivered" : "Pending", icon: ImageIcon, color: project.status === "COMPLETED" ? "fuchsia" : "amber" },
                ].map(({ label, status, icon: Icon, color }) => (
                  <div key={label} className={`flex items-center gap-4 p-4 rounded-xl border bg-card ${
                    color === "green" ? "border-green-500/20 bg-green-500/5" :
                    color === "teal" ? "border-teal-500/20 bg-teal-500/5" :
                    color === "fuchsia" ? "border-fuchsia-500/20 bg-fuchsia-500/5" :
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Recent Activity */}
              <div>
                <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-widest mb-3">Recent Activity</h3>
                <div className="space-y-0 bg-card border border-white/10 rounded-xl overflow-hidden">
                  {[
        ...
        {activeTab === "My Galleries" && (
          <motion.div key="galleries" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6" ref={galleryRef}>

            {/* Gallery Delivered Badge */}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 flex items-end justify-between opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button
                  ...
                  className={`w-11 h-11 rounded-xl flex items-center justify-center border transition-all ${
                    likedPhotos.has(heroIdx)
                      ? "bg-rose-500/30 border-rose-500/60 text-rose-400"
                      : "bg-black/40 backdrop-blur border-white/20 text-white"
                  }`}
                >
              ...
              {MOCK_PHOTOS.map((url, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`absolute inset-0 transition-all ${isHero ? "bg-fuchsia-900/10" : "bg-black/0 group-hover:bg-black/10"}`} />
                    <button
                      ...
                      className={`absolute top-1.5 right-1.5 w-6 h-6 rounded-full flex items-center justify-center transition-all opacity-0 group-hover:opacity-100 ${
                        likedPhotos.has(i) ? "bg-rose-500/80 text-white opacity-100" : "bg-black/50 text-white"
                      }`}
                    >
            ...
            <div className="relative">
              ...
              <div className="space-y-6">
                {TIMELINE_EVENTS.map((event, idx) => {
                  const colorMap: Record<string, { dot: string; border: string; badge: string; btn: string; preview: string }> = {
                    fuchsia: {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-col items-center shrink-0 pt-5">
                        <div className={`w-4 h-4 rounded-full border-2 border-background ${c.dot} z-10 shrink-0`} />
                      </div>
                        ...
                        <div className="flex flex-col md:flex-row md:items-start gap-4">
                          ...
                          <div className="flex gap-2 shrink-0">
                            {event.photoIndices.map((pi, i) => (
                              <div
                                key={i}
                                className={`w-20 h-20 rounded-xl overflow-hidden ring-1 ${c.preview} cursor-pointer hover:opacity-80 transition-opacity`}
            ...
            <div className="bg-card border border-white/10 rounded-2xl overflow-hidden">
                  ...
                  <div key={name} className="flex items-center justify-between p-3 rounded-xl bg-black/30 border border-white/5">
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${signed ? "bg-green-500/15" : "bg-amber-500/15"}`}>
                        {signed ? <Pen className="w-3.5 h-3.5 text-green-400" /> : <Clock className="w-3.5 h-3.5 text-amber-400" />}
                      </div>
                      <div>
              ...
              <div className="px-5 pb-5">
                <button className="w-full flex items-center justify-center gap-2 py-3 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 text-white text-sm font-semibold rounded-xl transition-all">
                  <Eye className="w-4 h-4" /> View Full Contract
