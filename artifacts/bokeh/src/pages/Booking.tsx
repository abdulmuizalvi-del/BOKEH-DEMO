    ...
    <AppLayout>
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-12 relative">
          ...
          <div 
            className="absolute left-0 top-1/2 -translate-y-1/2 h-0.5 bg-gradient-primary z-0 transition-all duration-500" 
          ...
          {STEPS.map((step, idx) => {
            ...
            return (
              <div key={step} className="relative z-10 flex flex-col items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-colors duration-500 shadow-lg ${
                  isActive ? 'bg-fuchsia-500 text-white shadow-fuchsia-500/50' : 
                  isCompleted ? 'bg-fuchsia-500 text-white' : 'bg-card border-2 border-white/20 text-white/50'
                }`}>
                  {isCompleted ? <CheckCircle2 className="w-4 h-4" /> : idx + 1}
                </div>
                <span className={`text-xs font-medium absolute -bottom-6 whitespace-nowrap ${isActive || isCompleted ? 'text-white' : 'text-white/40'}`}>
                  {step}
                </span>
              </div>
            );
            ...
            >
              {currentStep === 0 && (
                <div className="space-y-6 flex-1">
                  ...
                  <div className="space-y-2">
                    ...
                    <input type="date" className="w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 text-white color-scheme-dark" />
                  </div>
                </div>
              )}
              ...
              {currentStep === 1 && (
                <div className="space-y-6 flex-1">
                  ...
                  <textarea 
                    rows={6}
                    placeholder="Hi, I'm looking to book a portrait session for next month..."
                    className="w-full bg-black/40 border border-white/10 rounded-xl p-4 text-white focus:outline-none focus:border-fuchsia-500 resize-none"
                  />
                </div>
              )}
              ...
              {currentStep === 2 && (
                <div className="space-y-6 flex-1">
                  ...
                  <div className="h-40 bg-black/40 border border-white/10 rounded-xl p-4 overflow-y-auto text-xs text-white/60 font-mono">
                    1. SERVICES. Photographer agrees to provide photography services...
                    <br/><br/>2. PAYMENT. Client agrees to pay the total fee of $250...
                    <br/><br/>3. CANCELLATION. If Client cancels within 48 hours...
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-white/80">Type your full name to sign</label>
                    <input type="text" placeholder="John Doe" className="w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 text-white font-signature" />
                  </div>
                </div>
              )}
              ...
              {currentStep === 3 && (
                <div className="space-y-6 flex-1 flex flex-col items-center justify-center text-center">
                  ...
                  <button className="mt-4 px-8 py-3 bg-white text-black font-bold rounded-xl hover:bg-gray-200 transition-colors w-full sm:w-auto">
                    Pay with Stripe
                  </button>
                </div>
              )}
        ...
        <div className="flex justify-between mt-6">
          <button 
            ...
            className="px-6 py-2.5 rounded-xl font-medium text-white/70 hover:text-white hover:bg-white/5 disabled:opacity-0 transition-all"
          >
            Back
          </button>
          <button 
            ...
            className="px-6 py-2.5 rounded-xl font-semibold bg-gradient-primary text-white flex items-center gap-2 disabled:opacity-50 hover:shadow-[0_0_15px_rgba(217,70,239,0.4)] transition-all"
          >
