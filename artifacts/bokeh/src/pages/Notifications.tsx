...
import { Search, Check, MessageSquare, Folder, FileText } from "lucide-react";
import { motion } from "framer-motion";

export default function Notifications() {
  const { data: notifications, isLoading } = useNotifications();

  // Grouping logic is simplified since mock data already has groups
  ...
  const getIcon = (type: string) => {
    switch(type) {
      ...
      case 'project': return <Folder className="w-4 h-4 text-fuchsia-400" />;
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Stay updated on your projects and messages.</p>
          </div>
        ...
        <div className="flex gap-4 mb-8 border-b border-white/10 pb-4">
          {['All', 'Unread', 'Messages', 'Projects', 'Contracts'].map((f, i) => (
            <button key={f} className={`text-sm font-semibold transition-colors ${i === 0 ? 'text-fuchsia-400' : 'text-muted-foreground hover:text-white'}`}>
              {f}
            </button>
          ))}
        </div>
                ...
                <div className="space-y-3">
                  {items.map((notif, idx) => (
                    <motion.div 
                      ...
                      key={notif.id} 
                      className={`flex gap-4 p-4 rounded-xl border transition-colors ${notif.unread ? 'bg-white/5 border-fuchsia-500/30 shadow-[inset_4px_0_0_rgba(217,70,239,1)]' : 'bg-transparent border-white/5 hover:bg-white/5'}`}
                    >
                      ...
                      {notif.unread && (
                        <div className="w-2 h-2 rounded-full bg-fuchsia-500 self-center shadow-[0_0_8px_rgba(217,70,239,0.8)]" />
                      )}
                    </motion.div>
                  ))}
                </div>
