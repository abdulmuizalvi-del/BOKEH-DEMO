...
const STYLE_CATEGORIES = [
  { id: 'cinematic', label: 'Cinematic', color: 'from-amber-700 to-orange-600', img: 'https://images.unsplash.com/photo-1606800052052-a08af7148866?w=400&q=80' },
  ...
  { id: 'moody', label: 'Moody', color: 'from-stone-700 to-zinc-700', img: 'https://images.unsplash.com/photo-1511285560929-80b456fea0bc?w=400&q=80' },
  { id: 'editorial', label: 'Editorial', color: 'from-rose-700 to-pink-700', img: 'https://images.unsplash.com/photo-1583939003579-730e3918a45a?w=400&q=80' },
  { id: 'warm-lifestyle', label: 'Warm Lifestyle', color: 'from-orange-600 to-yellow-600', img: 'https://images.unsplash.com/photo-1469334031218-e382a71b716b?w=400&q=80' },
  { id: 'film-look', label: 'Film Look', color: 'from-teal-700 to-cyan-700', img: 'https://images.unsplash.com/photo-1532712938310-34cb3982ef74?w=400&q=80' },
]
...
export default function Discover() {
  ...
  return (
    <>
      <div className="mb-8">
        <h1 className="text-2xl md:text-3xl font-bold text-white mb-1">Discover Photographers</h1>
        <p className="text-white/40">Find the perfect creative for your vision.</p>
      </div>
      ...
      <div className="flex gap-3 mb-8">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full border border-white/10 rounded-xl py-3 pl-12 pr-4 text-white placeholder:text-white/30 focus:outline-none focus:ring-1 transition-all"
            style={{ background: 'rgba(30,18,12,0.6)' }}
          />
        </div>
        <button className="flex items-center gap-2 px-4 py-3 border border-white/10 text-white rounded-xl hover:border-white/20 hover:bg-white/5 transition-all" style={{ background: 'rgba(30,18,12,0.6)' }}>
      ...
      {/* Style Categories */}
      <div className="mb-10">
        <h2 className="text-lg font-bold text-white mb-4">Browse by Style</h2>
        <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
          {STYLE_CATEGORIES.map(cat => (
            <button
              key={cat.id}
              onClick={() => setSelectedStyle(selectedStyle === cat.id ? null : cat.id)}
              className={`relative rounded-xl overflow-hidden h-20 group transition-all ${selectedStyle === cat.id ? 'ring-2 ring-amber-600 scale-[1.02]' : 'hover:scale-[1.02]'}`}
            >
      ...
      {/* Photographers Grid */}
      <div>
        <h2 className="text-lg font-bold text-white mb-4">Featured Photographers</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-6">
          {PHOTOGRAPHERS.map((p, idx) => (
            <motion.div
              ...
              className="group border border-white/5 rounded-2xl overflow-hidden hover:border-white/20 transition-all cursor-pointer"
              style={{ background: 'rgba(30,18,12,0.6)' }}
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-xs text-white/40">({p.reviews})</span>
                  </div>
                  <span className="text-sm font-bold" style={{ color: '#d4851a' }}>from {p.price}</span>
                </div>
              </div>
            </motion.div>
          ))}
