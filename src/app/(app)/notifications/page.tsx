    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex gap-4 mb-8 border-b border-white/10 pb-4">
        {['All', 'Unread', 'Messages', 'Projects', 'Contracts'].map((f, i) => (
          <button key={f} className={`text-sm font-semibold transition-colors ${i === 0 ? 'text-fuchsia-400' : 'text-muted-foreground hover:text-white'}`}>{f}</button>
        ))}
      </div>
              ...
              <div className="space-y-3">
                {items.map((notif, idx) => (
                  ...
                  >
                    ...
                    {notif.unread && <div className="w-2 h-2 rounded-full bg-fuchsia-500 self-center shadow-[0_0_8px_rgba(217,70,239,0.8)]" />}
                  </motion.div>
                ))}
              </div>
            </div>
