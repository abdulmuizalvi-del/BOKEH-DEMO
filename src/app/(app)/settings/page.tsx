        ...
        <div className="w-full md:w-64 space-y-1 shrink-0">
          {TABS.map(tab => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium text-sm ${activeTab === tab.name ? 'bg-fuchsia-500/10 text-fuchsia-400 border border-fuchsia-500/20' : 'text-muted-foreground hover:bg-white/5 hover:text-white border border-transparent'}`}
            >
          ...
          {activeTab === 'Profile' && (
            <div className="space-y-8 max-w-2xl">
              <div>
                ...
                <div className="flex items-center gap-6 mb-8">
                  <div className="w-24 h-24 rounded-full bg-gradient-primary p-1">
                    <img src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&q=80" alt="Avatar" className="w-full h-full rounded-full object-cover border-2 border-card" />
                  </div>
                  <div>
              ...
              <div className="pt-6 border-t border-white/10 flex justify-end">
                <button className="px-6 py-2.5 bg-gradient-primary text-white font-semibold rounded-xl hover:shadow-[0_0_15px_rgba(217,70,239,0.4)] transition-all">Save Changes</button>
              </div>
            </div>
          )}
          {activeTab !== 'Profile' && (
