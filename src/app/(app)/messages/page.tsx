      ...
      <div className={`w-full md:w-[320px] border-r border-white/10 flex flex-col bg-background/50 ${activeUserId ? 'hidden md:flex' : 'flex'}`}>
        <div className="p-4 border-b border-white/10">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Search..." className="w-full bg-black/40 border border-white/10 rounded-lg py-2 pl-9 pr-4 text-sm text-white focus:outline-none focus:border-fuchsia-500 transition-colors" />
          </div>
          ...
          {listLoading ? (
            ...
            messagesList?.map(msg => (
              ...
              >
                ...
                {msg.unread && <div className="w-2.5 h-2.5 bg-fuchsia-500 rounded-full self-center shrink-0 shadow-[0_0_8px_rgba(217,70,239,0.8)]" />}
              </div>
            ))
          )}
        </div>
              ...
              <AnimatePresence>
                {activeThread.history.length === 0 ? (
                  ...
                  activeThread.history.map((msg, i) => (
                    ...
                    >
                      <div className={`p-4 rounded-2xl text-sm ${msg.sender === 'me' ? 'bg-gradient-primary text-white rounded-tr-sm shadow-[0_5px_15px_rgba(217,70,239,0.15)]' : 'bg-white/5 text-white/90 rounded-tl-sm border border-white/5'}`}>
                        {msg.text}
                      </div>
